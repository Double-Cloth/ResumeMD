(function (root, factory) {
  ...
  function buildBasicInfoItem(label, content, href) {
    const value = href
      ? '<a class="basic-info-value" href="' + escapeHTML(href) + '">' + escapeHTML(content) + '</a>'
      : '<span class="basic-info-value">' + escapeHTML(content) + '</span>';

    return '<div class="basic-info-item">' +
 ...
 function buildContactItem(content, href, type) {
   var iconSVG = getContactIcon(type);
   return href
 ...
 function buildResumeHTML(profile, bodyHTML) {
   ...
   var headerMetaKeys = { education: true, experience: true };

   Object.keys(data).forEach(function (key) {
     if (key === 'name' || key === 'title' || key === 'photo') return;
     var raw = data[key];
     ...
     if (key === 'email') {
       var val = clean(raw);
       if (!val) return;
       var href = sanitizeURL('mailto:' + val);
       items.push(buildContactItem(val, href, 'email'));
       return;
     }

     if (key === 'website') {
       var ws = normalizeWebsite(raw);
       if (!ws) return;
   ...
   let header = '';
    ...
    return header + '<main class="resume-body">' + profileSection + String(bodyHTML == null ? '' : bodyHTML) + '</main>';
  }
