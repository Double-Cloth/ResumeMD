...
- Markdown 编辑区提供 Front Matter、标题、列表、强调、代码和链接语法高亮。
- 顶部“说明”对话框集中展示 ResumeMD 的 Front Matter 和条目排版约定。
- 多页 A4 预览，长简历会自动分页，打印时保持页面拆分。
- 模板插入辅助，快速补充个人信息、教育背景、实习经历、项目经历、技能和荣誉模块。
- 字符数、模块数、页数实时统计，便于控制简历篇幅。
- 预览缩放与“适应宽度”控制，移动端会自动完整显示 A4 页面。
- 导入、导出 `.md` 文件，支持浏览器打印为 PDF。
- `localStorage` 自动保存，刷新页面后继续编辑。
- 导入、清空或恢复示例前自动保留上一份草稿，可跨刷新恢复或切回。
- 桌面双栏与移动端编辑/预览切换。
...
- 点击“说明”可随时查看章节、条目、日期配对和标题层级语法，不会修改当前草稿。
- 点击“清空”会清空当前草稿，并自动保存空白内容；下次打开仍保持空白。
- 点击“导入”可读取本地 `.md`、`.markdown` 或 `.txt` 文件。
- 导入、清空或恢复示例后，可点击编辑器底部的“恢复上份”交换当前草稿与上一份草稿。
- 点击“导出 MD”会把当前编辑器内容保存为 Markdown 文件；通过页面上传的照片会自动内嵌，便于在其他浏览器导入。
...
简历页眉按信息层级自动整理：电话、邮箱、所在地和个人主页归入“联系方式”，`education` 与 `experience` 归入“核心资历”，`availability`、`work_mode` 及其他可选字段归入“补充信息”。页眉会根据有效字段数量在精简、均衡和丰富布局之间自动切换，信息少时缩短纵向空间，信息多时让补充信息使用整行宽度；头像尺寸与间距也会随布局调整。`location` 与 `city` 同值时会自动去重；未填写的分组和照片区域不会占用空间。
...
- 文件最大 2 MiB，可容纳内嵌 1 MiB 照片的便携备份。
- 文件内容会覆盖当前编辑器内容。
